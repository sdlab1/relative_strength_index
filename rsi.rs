...
impl WilderRSI {
    ...
    // 
    // Формулы Wilder's Smoothing и SMMA идентичны при применении к RSI.
    // В Pine Script (TradingView) функция rma реализует именно этот метод и используется для RSI, что подтверждает их эквивалентность.
    // Различия могут возникать в других контекстах (например, разные периоды или интерпретации), но для RSI это одно и то же.
    // Вывод: В данном случае Wilder's Smoothing = SMMA = RMA. Отличий нет, это синонимы для метода сглаживания, используемого в RSI.
